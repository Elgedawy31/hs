...
// Get metrics operation for months
export const getActivityMetricsForMonths = createAsyncThunk(
  "activity/getActivityMetricsForMonths",
  async ({ token, from, to, userId }, { rejectWithValue }) => {
    try {
      const response = await fetch(
        `${API_URL}/${KEY}/metrics?from=${from}&to=${to}&userId=${userId}`,
    ...
    try {
      ...
      if (!data.activities) {
        return rejectWithValue(
          data?.error || data?.message || "Failed to get activities"
        );
      }
      return data;
    } catch (err) {
...
export const getOneActivity = createAsyncThunk(
  ...
  async ({ activityId, token }, { rejectWithValue }) => {
    try {
      ...
      if (!data.success) {
        return rejectWithValue(
          data?.error || data?.message || "Failed to get activity"
        );
      }
...
const activitySlice = createSlice({
  ...
  initialState: {
    ...
    metricsError: null,
    metricsLoadingForCards: false,
    metricsErrorForCards: null,
    metricsLoadingForMonths:false,
    metriscErrorForMonths:null,
  },
  reducers: {
    resetActivityState: (state) => {
      ...
      (state.metricsError = null),
        ...
        (state.metricsLoadingForCards = false),
        (state.metricsErrorForCards = null),
        (state.metricsLoadingForMonths = false),
        (state.metriscErrorForMonths = false);
    },
  },
  extraReducers: (builder) => {
    builder
      ...
      .addCase(deleteActivity.fulfilled, (state, action) => {
        ...
        if (
          state.currentActivity &&
          state.currentActivity._id === action.payload.activityId
        ) {
      ...
      .addCase(getActivityMetricsForCards.pending, (state) => {
        state.metricsLoadingForCards = true;
        state.metricsErrorForCards = null;
      })
      ...
      .addCase(getActivityMetricsForCards.rejected, (state, action) => {
        state.metricsLoadingForCards = false;
        state.metricsErrorForCards = action.payload;
      })
      // Get activity metrics for months
      .addCase(getActivityMetricsForMonths.pending, (state) => {
        state.metricsLoadingForMonths = true;
        state.metriscErrorForMonths = null;
      })
      .addCase(getActivityMetricsForMonths.fulfilled, (state, action) => {
        state.metricsLoadingForMonths = false;
        state.metricsForMonths = action.payload;
        state.metriscErrorForMonths = null;
      })
      .addCase(getActivityMetricsForMonths.rejected, (state, action) => {
        state.metricsLoadingForMonths = false;
        state.metriscErrorForMonths = action.payload;
      });
  },
});
